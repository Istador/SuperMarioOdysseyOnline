    ...
    if (mInfo->mIsPlayerIt) {
        ...
        if (mInvulnTime < 5) {
            ...
            for (size_t i = 0; i < (size_t)mPuppetHolder->getSize(); i++) {
                ...
                float pupDistSq = vecDistanceSq(other->playerPos + offset, playerPos); // TODO: remove distance calculations and use hit sensors to determine this

                if (   pupDistSq < 40000.f // non-squared: 200.0
