...
void FreezeTagMode::update() {
    ...
    int connectedRunners = Client::isSocketActive() ? isPlayerRunner() : 0;
    int connectedChasers = Client::isSocketActive() ? isPlayerChaser() : 0;
    ...
    bool enoughChasers = chasers();
    bool enoughRunners = runners();
    ...
    if (isRound() && (!connectedChasers || !connectedRunners)) {
        mDisconnectTimer += Time::deltaTime;
        if (5.0f < mDisconnectTimer) { // 5 seconds grace period
            enoughChasers = connectedChasers;
            enoughRunners = connectedRunners;
