...
void FreezeTagMode::update() {
    ...
    if (mInfo->mIsRound && 3 <= mInvulnTime) {
        ...
        for (size_t i = 0; i < mPuppetHolder->getSize(); i++) {
            ...
            if (   mInfo->mIsPlayerFreeze       // we're a frozen runner
                && freezeMinTime <= mInvulnTime // since some time
                && isPlayerAlive                // that is alive
    ...
    if (   !mInfo->mIsRound          // not during a round
        ...
        && !mIsEndgameActive         // not in endgame
    ...
    // Verify that the standard HUD is hidden
    if (!mCurScene->mSceneLayout->isEnd()) {
        mCurScene->mSceneLayout->end();
    }

    // Spectate camera
    if (!mTicket->mIsActive && mInfo->mIsPlayerFreeze) { // enable spectate camera when frozen
        al::startCamera(mCurScene, mTicket, -1);
        al::requestStopCameraVerticalAbsorb(mCurScene);
    }
    if (mTicket->mIsActive && !mInfo->mIsPlayerFreeze) { // disable spectate camera when unfrozen
        al::endCamera(mCurScene, mTicket, 0, false);
        al::requestStopCameraVerticalAbsorb(mCurScene);
    }
    if (mTicket->mIsActive && mInfo->mIsPlayerFreeze) { // update spectate camera
