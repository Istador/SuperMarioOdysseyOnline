...
void FreezeTagMode::update() {
    ...
    // keep track of runner and chaser counts
    int runners          = mInfo->mIsPlayerRunner ? 1 : 0;
    int chasers          = mInfo->mIsPlayerRunner ? 0 : 1;
    int connectedRunners = Client::isSocketActive() ? runners : 0;
    int connectedChasers = Client::isSocketActive() ? chasers : 0;
    ...
    for (int i = 0; i < mPuppetHolder->getSize(); i++) {
        ...
        if (mInfo->mIsRound && !other->isFreezeInRound) {
            mInfo->mOtherPlayers.pushBack(other);
            continue;
        }
        ...
        if (other->isFreezeTagRunner) {
            runners += 1;
            connectedRunners += other->isConnected;
            mInfo->mRunnerPlayers.pushBack(other);
        } else {
            chasers += 1;
            connectedChasers += other->isConnected;
    ...
    if (mInfo->mIsRound && 3 <= mInvulnTime) {
        ...
        float freezeMinTime = mInfo->mIsPlayerRunner ? al::clamp(3.f + (mInfo->mFreezeCount * 0.5f), 3.f, 7.f) : 0.f; // cooldown of 3-7s, +0.5s per frozen runner
        ...
        for (size_t i = 0; i < mPuppetHolder->getSize(); i++) {
            ...
            if (mInfo->mIsPlayerRunner) { // if we're a runner
                // Check if the chaser freezes us
                bool chaserFreezesUs = (
                       !mInfo->mIsPlayerFreeze   // we're an unfrozen runner
                ...
                bool runnerUnfreezesUs = (
                       !chaserFreezesUs
                    && mInfo->mIsPlayerFreeze       // we're a frozen runner
            ...
            } else if (other->isFreezeTagRunner && !other->isFreezeTagFreeze) { // if we're a chaser, and the other player is a unfrozen runner
                float distanceSq = vecDistanceSq(playerPos, other->playerPos);
    ...
    // if enough chasers/runners have disconnected to prevent continuing this round => cancel it
    if (mInfo->mIsRound && (!connectedChasers || !connectedRunners)) {
        mDisconnectTimer += Time::deltaTime;
        if (5.0f < mDisconnectTimer) { // 5 seconds grace period
            chasers = connectedChasers;
            runners = connectedRunners;
    ...
    // Not enough chasers/runners to continue this round => cancel it
    if (mInfo->mIsRound && (!chasers || !runners)) {
        endRound(true);
