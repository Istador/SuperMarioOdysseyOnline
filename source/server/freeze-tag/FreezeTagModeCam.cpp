...
void FreezeTagMode::updateSpectateCam(PlayerActorBase* playerBase) {
    ...
    if (curPoser && al::isEqualString(curPoser->getName(), "CameraPoserActorSpectate")) {
        ...
        if (mIsEndgameActive && mSpectateIndex != -1) { indexDirection = -1; } // Move index left

        // Force index to decrease if our current index got out of bounds
        if (mInfo->mRunnerPlayers.size() <= mSpectateIndex) { indexDirection = -1; } // Move index left
        ...
        if (mPrevSpectateIndex == mSpectateIndex && mPrevSpectateCount == mInfo->mRunnerPlayers.size()) {
            return;
        }
        ...
        mPrevSpectateCount = mInfo->mRunnerPlayers.size();
    }
}
